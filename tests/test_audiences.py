...
def test_add_audiences(login_by_user_type):
    ...
    # AUDIENCE CREATION
    sidebar_navigation.navigate_to_audiences()
    ...
    with open('assets/audiences/audiences_audience_group_data.json') as json_file:
        audiences_audience_group_data = json.load(json_file)
    audiences_audience_group_data['general_information']['audience_name'] = \
        audiences_audience_group_data['general_information']['audience_name'] + generic_modules.get_random_string(5)
    audiences_audience_group_data['general_information']['audience_list'] = \
        audiences_audience_group_data['general_information']['audience_list'] + \
        audiences_behavioral_user_interests_data['general_information']['audience_name']
    # AUDIENCE CREATION
    sidebar_navigation.navigate_to_audiences()
    audiences_list_page.navigate_to_add_audience()
    audiences_form_page.provide_audience_group_audience_data_and_save(audiences_audience_group_data)
    assert "Audience created successfully!" in audiences_list_page.get_success_message()
    # DATA VERIFICATION
    base_page.click_on_specific_tab(tab_name=AudienceListLocators.dmp_audience_groups_tab)
    audiences_list_page.search_and_action(audiences_audience_group_data['general_information']['audience_name'],
                                          "Edit")
    pulled_audiences_audience_group_data_gui = audiences_form_page.get_audience_group_audience_information()
    assert "All data verification is successful" == CompareUtil.verify_data(pulled_audiences_audience_group_data_gui,
    ...
    audiences_list_page.search_and_action(
        audiences_behavioral_user_interests_data['general_information']['audience_name'], "Delete")
    assert "Successfully deleted 1 audiences" in audiences_list_page.get_success_message()
...
def test_add_audience_geolocation(login_by_user_type):
    config, driver = login_by_user_type
    audiences_list_page = DspDashboardAudienceList(driver)
    audiences_form_page = DspDashboardAudienceForm(driver)
    sidebar_navigation = DashboardSidebarPage(driver)
...
def test_add_audience_apps(login_by_user_type):
    config, driver = login_by_user_type
    audiences_list_page = DspDashboardAudienceList(driver)
    audiences_form_page = DspDashboardAudienceForm(driver)
    sidebar_navigation = DashboardSidebarPage(driver)
...
def test_add_audience_site_first_party(login_by_user_type):
    config, driver = login_by_user_type
    audiences_list_page = DspDashboardAudienceList(driver)
    audiences_form_page = DspDashboardAudienceForm(driver)
    sidebar_navigation = DashboardSidebarPage(driver)
    ...
    audiences_list_page.search_and_action(
        audiences_retargeting_site_first_party_data['general_information']['audience_name'], "Edit")
...
def test_add_audience_user_ids_list(login_by_user_type):
    config, driver = login_by_user_type
    audiences_list_page = DspDashboardAudienceList(driver)
    audiences_form_page = DspDashboardAudienceForm(driver)
    sidebar_navigation = DashboardSidebarPage(driver)
    ...
    base_page.click_on_specific_tab(tab_name=AudienceListLocators.first_party_tab)
    audiences_list_page.search_and_action(
        audiences_user_ids_list_data['general_information']['audience_name'], "Edit")
